...
export abstract class BaseRedisStorage implements IStorage {
  ...
  async deleteUser(userName: string): Promise<void> {
    ...
    // 删除永久搜索记录
    await this.withRetry(() => this.client.del(this.permanentSearchKey(userName)));

    // 删除永久收藏记录
    const permanentFavoritePattern = `u:${userName}:permanent-fav:*`;
    const permanentFavoriteKeys = await this.withRetry(() =>
      this.client.keys(permanentFavoritePattern)
    );
    if (permanentFavoriteKeys.length > 0) {
      await this.withRetry(() => this.client.del(permanentFavoriteKeys));
    }

    // 删除永久观影记录
    const permanentPlayPattern = `u:${userName}:permanent-play:*`;
    const permanentPlayKeys = await this.withRetry(() =>
      this.client.keys(permanentPlayPattern)
    );
    if (permanentPlayKeys.length > 0) {
      await this.withRetry(() => this.client.del(permanentPlayKeys));
    }
  }
  ...
  async addPermanentSearchRecord(userName: string, keyword: string, timestamp: number): Promise<void> {
    ...
    // 插入到列表末尾（保持时间顺序）
    await this.withRetry(() => this.client.rPush(key, record));
  }
  ...
  async getPermanentSearchRecords(userName: string, limit = 100): Promise<Array<{ keyword: string; timestamp: number }>> {
    ...
    const records = await this.withRetry(() => this.client.lRange(key, start, -1));
  ...
  async addPermanentFavoriteRecord(userName: string, favorite: Favorite): Promise<void> {
    ...
    await this.withRetry(() => this.client.set(itemKey, JSON.stringify(favorite)));
  }
  ...
  async getPermanentFavoriteRecords(userName: string, limit = 100): Promise<Favorite[]> {
    ...
    keys.forEach((fullKey: string, idx: number) => {
      ...
      if (raw) {
        try {
          const favorite = JSON.parse(raw) as Favorite;
  ...
  async addPermanentPlayRecord(userName: string, playRecord: PlayRecord): Promise<void> {
    ...
    await this.withRetry(() => this.client.set(itemKey, JSON.stringify(playRecord)));
  }
  ...
  async getPermanentPlayRecords(userName: string, limit = 100): Promise<PlayRecord[]> {
    ...
    keys.forEach((fullKey: string, idx: number) => {
      ...
      if (raw) {
        try {
          const playRecord = JSON.parse(raw) as PlayRecord;
